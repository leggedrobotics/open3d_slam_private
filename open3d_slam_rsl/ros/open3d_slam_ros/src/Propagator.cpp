...
Propagator::Propagator(const rclcpp::NodeOptions& options)
    : rclcpp::Node("propagator", options),
      tf_buffer_(std::make_shared<tf2_ros::Buffer>(this->get_clock())),
      tf_listener_(std::make_shared<tf2_ros::TransformListener>(*tf_buffer_)) {}

void Propagator::initCommonRosStuff() {
  map_pose_topic_ = this->declare_parameter<std::string>("map_pose_topic", "/rslidar_points");
  fast_pose_topic_ = this->declare_parameter<std::string>("fast_pose_topic", "/state_estimator/pose_in_odom");
  distance_cutoff_ = this->declare_parameter<float>("distance_cutoff", 0.5);

  std::cout << "[Propagator] map_pose_topic param: " << map_pose_topic_ << '\n';
  std::cout << "[Propagator] fast_pose_topic param: " << fast_pose_topic_ << '\n';
  std::cout << "[Propagator] distance_cutoff param: " << distance_cutoff_ << '\n';

  processedPub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("propagator_deskewed_cloud", rclcpp::QoS(1).transient_local());
}

void Propagator::subscribePoses() {
  ...
  bool ready = tf_buffer_->canTransform("base", "rslidar", tf2::TimePointZero, tf2::durationFromSec(TF_TIMEOUT));
    ...
    if (ready) {
      std::cout << "\033[1;32m[Propagator] Found static TF base↔rslidar.\033[0m" << std::endl;
      break;
    }
    ...
    if ((now - start_time) >= warn_duration) {
      std::cout << "\033[1;33m[Propagator] Waiting for static TF base↔rslidar...\033[0m" << std::endl;
      start_time = now;
    }
  }
  ...
  try {
    ...
    RCLCPP_WARN(this->get_logger(), "\033[1;33m[Propagator] This exception is thrown on purpose if the transform is missing!\033[0m");
  ...
  airySub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&Propagator::cloudCallback, this, std::placeholders::_1));

  std::cout << "\033[1;32m[Propagator] Subscribed to cloud topic successfully.\033[0m" << std::endl;
  ...
  std::cout << "\033[1;32m[Propagator] Subscribed to pose topic successfully.\033[0m" << std::endl;
}

void Propagator::cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg) {
...
Propagator::FieldOffsets Propagator::computeFieldOffsets(const sensor_msgs::msg::PointCloud2& msg) const {
  FieldOffsets o;
...
void Propagator::isom3d_to_mat33_vec3(const Eigen::Isometry3d& iso, double R[3][3], double t[3]) {
  auto m = iso.linear();
  for (int r = 0; r < 3; ++r)
...
sensor_msgs::msg::PointCloud2 Propagator::deskewPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg) {
